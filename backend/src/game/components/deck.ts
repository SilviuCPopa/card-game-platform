import { CARD_COLOR, CARD_FORM, INITIAL_CARD_INDEX, LAST_CARD_INDEX } from "../properties/card.properties";
...
export class Deck {
    ...
    private createCardsWithForm(form: CARD_FORM, color?: CARD_COLOR) {
        for(let i = INITIAL_CARD_INDEX; i <= LAST_CARD_INDEX; i++) {
            this.cards.push(new Card(i.toString(), form, color));
        }
    }
} 