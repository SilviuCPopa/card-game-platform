...
export class GameActionHandler {
        ...
        if (players) {
            Object.values(players).forEach( (player: any) => {
                clients[player.getId()].emit({
                    key: action.key,
                    value: this.getRoundPayload(players, player);
                });
            })
        }
    }
    ...
    private getRoundPayload(players: any, player: Player) {
        return {
            player: player,
            oponent: {
                handLength: this.getOponent(players, player).getHand().getHandSize()
            }
        }
    }
}