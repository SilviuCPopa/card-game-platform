...
export class GameSocket implements Observer {
    ...
    initialize() {
        this.socket.on('connection', (clientSocket: any) => {
            ...
            console.log(Object.keys(this.gameSockets).length < 2, clientSocket.handshake.socketId);
            if (Object.keys(this.gameSockets).length < 2 || !!this.gameSockets[clientSocket.handshake.socketId]) {
