...
export class Hand {
    ...
    hasCandidateCards(card: Card) {
        const candidateCards = this.getCandidateCards(card);
        return !!candidateCards.length;
    }

    getCandidateCards(card: Card) {
        return Object.values(this.cards).filter(item => (item.getName() === card.getName()) || this.isWinnerCard(card));
    }
